...
int soundData[MAXSOUNDBUFFER];	// sound data. Legend: lower part: TState where the out was generated. Bit 18 -> out #fe,0 ->0  out #fe,16 -> 1
int currentSoundPtr=-1;		// place in the soundData buffer where we'll store next out #fe
...
void initSoundLog (void)
{
   if (currentSoundPtr==-1)	// first time initialization
   {
      soundData[0]=0;
      currentSoundPtr=1;
    } else {	               // at least this is the second frame
...
int val;
	...
	if (currentSoundPtr>=MAXSOUNDBUFFER) return;		// no space, no log (should never happen, buffer is big enough)
	if (state>0) state=262144;		// 0 or 16 -> 0 or 262144

	if ((soundData[currentSoundPtr-1]&262144)==state) return;	// no need to log if it's the same state than before
...
//FILE *f;
	...
	if (!gSoundInited) return;				// no init -> no sound -> bye

	// create sound, 44100, mono, 8 bits unsigned

	cur_sound=soundData[0]&262144;			// get current beeper status
	if (cur_sound) cur_sound=255;			// convert it to 0 or 255 (unsigned sound data)
	else cur_sound=0;
...
if (beeperSound!=NULL)
{
	...
	for (i=1;i<currentSoundPtr;i++)			// for each logged port data
	{
		...
		for (;cur<j;cur++) beeperSound[cur]=cur_sound;		// fill the buffer with current status until a new out #fe appears

		cur_sound=soundData[i]&262144;		// get next status
		if (cur_sound) cur_sound=255;
		else cur_sound=0;
	}

	// end filling all the buffer till its end
	for (;cur<882;cur++) beeperSound[cur]=cur_sound;
}
