...
------------------------------------------------------------------*/
void GetHexValue( int x, int y, char *str, char *result,
...
------------------------------------------------------------------*/
void DrawInstruction( Z80Regs *regs, char *font )
{
    char b[1024];
...
------------------------------------------------------------------*/
void ShowMem( Z80Regs *regs, int offset, char *font )
{
    char b[256];
    int i;
//    char instruccion[256];
    ...
    for(i=0; i<9; i++ )
    {
        sprintf(b, "%04X %04X", offset+(i*2),
              (Z80MemRead(offset+(i*2), regs))+ (Z80MemRead(offset+(i*2)+1, regs)<<8));
    ...
    for(i=0; i<3; i++ )
    {
       sprintf(b, "%04X %04X", regs->SP.W+(i*2),
              (Z80MemRead(regs->SP.W+(i*2), regs)) + (Z80MemRead(regs->SP.W+(i*2)+1, regs)<<8) );
    ...
    for(i=0; i<2; i++ )
    {
       sprintf(b, "%04X %04X", regs->PC.W+(i*2),
              (Z80MemRead(regs->PC.W+(i*2), regs)) + (Z80MemRead(regs->PC.W+(i*2)+1, regs)<<8) );
...
------------------------------------------------------------------*/
void Z80Dump( Z80Regs *regs, char *font )
{
  char b[1024];
   ...
   int fgcolor = 0;
    ...
    for(i=0; i<NUM_DEBUGGER_HELPLINES; i++)
      GFXprintf_tovideo( menux+12, (fonth*i)+32, 
                         lang_debugger_text[(language*NUM_DEBUGGER_HELPLINES)+i],
