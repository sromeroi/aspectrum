...
/*unsigned short int Pixeles[192];
unsigned short int Atributos[192];
*/
...
// codigo de control de argumentos pasados al programa. 
...
#endif
   {
      switch (c)
      {
         case 'r': strncpy(emuopt.romfile,optarg,255);
                 break;
...
//  Comprobamos y abrimos el fichero de cinta si procede.
	if (emuopt.tapefile[0]==0) printf("Not using tape.\n");
	else if ((fp=fopen(emuopt.tapefile,"rb")) !=NULL) 
	{
...
//  Comprobamos y abrimos el snapshot si procede.
	if (emuopt.snapfile[0]==0) printf("No loading snapshot.\n");
   ...
   while( !done )
   {
      ...
      switch(tecla >> 8)
      {
         ...
         case gKEY_F5 : Z80Reset( &spectrumZ80, 69888 );
                        tecla = gKEY_ESC << 8; 
                        debug = 1-debug;
                        break;
         ...
         case gKEY_F8 : if( language < LANGUAGES-1 )
                           language++;
                        else
                           language = 0;
                        tecla = gKEY_ESC << 8;
...
//                  blit( vscreen, screen, 0, scanl+4, 0, scanl+4, 320, 1);
       ...
       case 1:
             switch( tecla & 0xFF )
             {
                ...
                case 't':  GetHexValue(2,130, lang_runto_t[language],
                                       b, tfont, 6, 0, 6);
...
*/
                ...
                // Poke memory
                case 'p':  GetHexValue(2, 130, lang_poke[language],
                                       b, tfont,6,0,6);
                           poke = strtol(b, (char**)NULL, 16 );
                           GetHexValue(2, 130, lang_value[language],
                                       b, tfont,6,0,6);
                ...
                case 'A':  GetHexValue(2,130, lang_change_af2[language], b, tfont,6,0,6);
                ...
                case 'B':  GetHexValue(2,130,lang_change_bc2[language], b, tfont,6,0,6);
                           ...
                           break;
                case 'b':  GetHexValue(2,130,lang_change_bc[language], b, tfont,6,0,6);
                           spectrumZ80.BC.W = strtol(b, (char**)NULL, 16 );
                           break;
                case 'D':  GetHexValue(2,130,lang_change_de2[language], b, tfont,6,0,6);
                           spectrumZ80.DEs.W = strtol(b, (char**)NULL, 16 );
                           break;
                case 'd':  GetHexValue(2,130,lang_change_de[language], b, tfont,6,0,6);
                           spectrumZ80.DE.W = strtol(b, (char**)NULL, 16 );
                           break;
                case 'H':  GetHexValue(2,130,lang_change_hl2[language], b, tfont,6,0,6);
                ...
                case 'y':  GetHexValue(2,130,lang_change_iy[language], b, tfont,6,0,6);
                           ...
                           break;
                case 'S':  GetHexValue(2,130,lang_change_sp[language], b, tfont,6,0,6);
                           spectrumZ80.SP.W = strtol(b, (char**)NULL, 16 );
                           break;
                case 'n':  if( offs > 0) offs-=2;
                           ShowMem( &spectrumZ80, offs, tfont );
                           break;
                case 'm':  if( offs < 0xFFFF - 18 ) offs+=2;
                           ShowMem( &spectrumZ80, offs, tfont );
                           break;
                case 'l':  GetHexValue(2,130, lang_change_mem[language], b, tfont,6,0,6);
...
static unsigned char teclas_fila[NUM_KEYB_KEYS][3] =
{
  ...
  { 1,2,0xEF }, /*CTRL*/
   ...
   if( gkey[KEY_C] ) fila[4][1] &= (0xF7);
   if( gkey[KEY_V] ) fila[4][1] &= (0xEF);
   if( gkey[KEY_RSHIFT] || key[KEY_LSHIFT] ) 
                     fila[4][1] &= (0xFE);

   if( gkey[KEY_A] ) fila[3][1] &= (0xFE);
   if( gkey[KEY_S] ) fila[3][1] &= (0xFD);
   if( gkey[KEY_D] ) fila[3][1] &= (0xFB);
   if( gkey[KEY_F] ) fila[3][1] &= (0xF7);
   if( gkey[KEY_G] ) fila[3][1] &= (0xEF);

   if( gkey[KEY_Q] ) fila[2][1] &= (0xFE);
   if( gkey[KEY_W] ) fila[2][1] &= (0xFD);
   if( gkey[KEY_E] ) fila[2][1] &= (0xFB);
   if( gkey[KEY_R] ) fila[2][1] &= (0xF7);
   if( gkey[KEY_T] ) fila[2][1] &= (0xEF);

   if( gkey[KEY_1] ) fila[1][1] &= (0xFE);
   if( gkey[KEY_2] ) fila[1][1] &= (0xFD);
   if( gkey[KEY_3] ) fila[1][1] &= (0xFB);
   if( gkey[KEY_4] ) fila[1][1] &= (0xF7);
   if( gkey[KEY_5] ) fila[1][1] &= (0xEF);

   if( gkey[KEY_0] ) fila[1][2] &= (0xFE);
   if( gkey[KEY_9] ) fila[1][2] &= (0xFD);
   if( gkey[KEY_8] ) fila[1][2] &= (0xFB);
   if( gkey[KEY_7] ) fila[1][2] &= (0xF7);
   if( gkey[KEY_6] ) fila[1][2] &= (0xEF);

   if( gkey[KEY_P] ) fila[2][2] &= (0xFE);
   if( gkey[KEY_O] ) fila[2][2] &= (0xFD);
   if( gkey[KEY_I])  fila[2][2] &= (0xFB);
   if( gkey[KEY_U] ) fila[2][2] &= (0xF7);
   if( gkey[KEY_Y] ) fila[2][2] &= (0xEF);

   if( gkey[KEY_ENTER] ) fila[3][2] &= (0xFE);
   if( gkey[KEY_L] )     fila[3][2] &= (0xFD);
   if( gkey[KEY_K] )     fila[3][2] &= (0xFB);
   if( gkey[KEY_J] )     fila[3][2] &= (0xF7);
   if( gkey[KEY_H] )     fila[3][2] &= (0xEF);

   if( gkey[KEY_SPACE] ) fila[4][2] &= (0xFE);
   if( gkey[KEY_ALT] || key[KEY_ALT] ) 
                         fila[4][2] &= (0xFD);
   if( gkey[KEY_M] )     fila[4][2] &= (0xFB);
   if( gkey[KEY_N] )     fila[4][2] &= (0xF7);
   if( gkey[KEY_B] )     fila[4][2] &= (0xEF);

   if(gkey[KEY_BACKSPACE] ) { fila[4][1] &= (0xFE); fila[1][2] &= (0xFE); }
   if(gkey[KEY_TAB] ) { fila[4][1] &= (0xFE); fila[4][2] &= (0xFD); }
...
#define filas teclas_fila
   ...
   if( gkey[KEY_UP]   )     fila[filas[CUP][0]][filas[CUP][1]] &= (filas[CUP][2]);
   if( gkey[KEY_DOWN] )     fila[filas[CDOWN][0]][filas[CDOWN][1]] &= (filas[CDOWN][2]);
   if( gkey[KEY_RIGHT])     fila[filas[CRIGHT][0]][filas[CRIGHT][1]] &= (filas[CRIGHT][2]);
   if( gkey[KEY_LEFT] )     fila[filas[CLEFT][0]][filas[CLEFT][1]] &= (filas[CLEFT][2]);
   if( gkey[KEY_RCONTROL] ) fila[filas[FIRE][0]][filas[FIRE][1]] &= (filas[FIRE][2]);
